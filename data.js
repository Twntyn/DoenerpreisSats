...
// Dynamisch aktuellen BTC-Preis abrufen und 2025er Sats-Datenpunkt hinzufügen
async function addCurrentDataPoint() {
  try {
    const response = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=eur');
    const data = await response.json();
    const btcEur = data.bitcoin.eur;

    const donerEur = 7.0; // Aktueller Dönerpreis in EUR für 2025 (festgelegt oder geschätzt)
